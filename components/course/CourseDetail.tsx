...
import {
  courseDuration, courseRating, formatDuration, formatPrice, getCategoryById,
} from '@/lib/data'
...
/**
 * コース詳細の表示。course を props で受け取る。
 *
 * 通常ページは slug から静的データを引いて渡し、プレビューページは microCMS の
...
export default function CourseDetail({ course, preview = false }: { course?: Course; preview?: boolean }) {
  ...
  if (!course || (!preview && !course.isPublished)) {
    return (
  ...
  const handlePrimary = () => {
    // 下書きプレビューではレッスンが未入力のことがある。押しても何も起きないようにする
